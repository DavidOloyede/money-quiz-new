// forked from design-sync lib/bundle.mjs — add esbuild jsx:'automatic' so synth
// bundling of this app's React-19 (automatic-runtime) .tsx works.
//
// Why this fork is needed: the components use the automatic JSX runtime (no
// `import React`), but the repo's root tsconfig.json is solution-style
// (references only, no compilerOptions.jsx), so esbuild's per-file tsconfig
// discovery finds no jsx setting and defaults to the CLASSIC transform
// (React.createElement) — which throws "React is not defined" at runtime and
// blanks every preview. There is no cfg knob for esbuild's jsx mode, so the
// only fix is here. The output contract (IIFE format, @ds-bundle header,
// globalName, sourceHashes) is UNCHANGED — only the .tsx→JS transform differs.
// On re-sync, diff this against the bundled lib/bundle.mjs and fold in changes.
...
export async function bundleToIife({ entry, globalName, nodePaths, out, tsconfig }) {
  ...
  try {
    buildResult = await build({
      ...
      platform: 'browser',
      target: 'es2020',
      // FORK: this repo's .tsx use React's automatic JSX runtime with no
      // discoverable tsconfig jsx setting — force it so esbuild emits
      // react/jsx-runtime imports (handled by reactShim) instead of the
