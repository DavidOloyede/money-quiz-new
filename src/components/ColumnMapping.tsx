...
function ColumnSelect({
  ...
  return (
    <select
      ...
      className="w-full rounded-lg border border-linen-300 dark:border-linen-600 px-3 py-2 text-sm focus:border-forest-500 focus:border-forest-500 focus:ring-2 focus:ring-forest-500/25 focus:outline-none focus:ring-1 focus:ring-forest-500"
    >
  ...
  return (
    <div className="rounded-xl border border-linen-200 dark:border-linen-700 bg-cream dark:bg-linen-900 p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="font-display font-semibold text-linen-800 dark:text-linen-100">Match your columns</h3>
            ...
            <button
              ...
              className={`rounded-lg border px-3 py-1.5 text-sm font-medium transition-colors ${
                (mapping.accountType ?? 'bank') === opt.id
                  ? 'border-forest-500 bg-forest-50 dark:bg-forest-500/10 text-forest-700 dark:text-forest-300'
                  : 'border-linen-300 dark:border-linen-600 text-linen-600 dark:text-linen-300 hover:bg-linen-50 dark:hover:bg-linen-800'
              }`}
            >
              {opt.label}
            </button>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        {mapping.amountMode === 'single' ? (
          <>
            ...
            <label className="flex items-center gap-2 self-end pb-2 text-sm text-linen-600 dark:text-linen-300">
              <input
                type="checkbox"
                checked={mapping.invertAmount ?? false}
                onChange={(e) => update({ invertAmount: e.target.checked })}
                className="h-4 w-4 rounded border-linen-300 dark:border-linen-600 text-forest-600 focus:ring-forest-500"
              />
      ...
      <div className="mt-6">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className="text-xs text-linen-500 dark:text-linen-400">
            ...
            {full.skipped > 0 && (
              <span className="text-honey-600"> · {full.skipped} skipped</span>
            )}
          </span>
        </div>
        <div className="overflow-x-auto rounded-lg border border-linen-100 dark:border-linen-800">
          <table className="w-full text-sm">
            <thead className="bg-linen-50 dark:bg-linen-800/50 text-left text-xs uppercase tracking-wide text-linen-400 dark:text-linen-500">
              <tr>
            ...
            <tbody className="divide-y divide-linen-100 dark:divide-linen-800">
              {preview.transactions.map((t) => (
                <tr key={t.id}>
                  <td className="whitespace-nowrap px-3 py-2 text-linen-500 dark:text-linen-400">
                    {formatDate(t.date)}
                  </td>
                  <td className="px-3 py-2 text-linen-700 dark:text-linen-200">{t.description}</td>
                  <td className="px-3 py-2 text-linen-600 dark:text-linen-300">
              ...
              {preview.transactions.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-3 py-6 text-center text-sm text-linen-400 dark:text-linen-500">
      ...
      <div className="mt-6 flex items-center justify-end gap-3">
        ...
        <button
          ...
          disabled={!valid}
          className="rounded-lg bg-forest-600 px-4 py-2 text-sm font-medium text-white hover:bg-forest-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
